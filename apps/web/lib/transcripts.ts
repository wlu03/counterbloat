// Data model + deterministic workflow simulation for the transcript
...
export const transcripts: TranscriptRecord[] = [
  {
    id: "ec_nmbs_q2fy26",
    ticker: "NMBS",
    ...
    sector: "software_ai",
    period: "Q2 FY2026",
    callDate: "2026-09-11T21:00:00Z",
    cik: "0001788028",
    docs: ["transcript", "press_release", "10-Q", "audio"],
    executiveCount: 3,
    analystCount: 9,
    wordCount: 11840,
    source: "Motley Fool archive",
    summary:
      "AI-inference platform. Heavy promotion of a new agentic runtime; guidance raised on GPU demand.",
    audioVintage: "current webcast replay",
  },
  {
    id: "ec_astr_q1fy26",
    ticker: "ASTR",
    ...
    sector: "software_ai",
    period: "Q1 FY2026",
    callDate: "2026-08-28T20:30:00Z",
    cik: "0001902141",
    docs: ["transcript", "press_release", "10-Q"],
    executiveCount: 2,
    analystCount: 11,
    wordCount: 13120,
    source: "8-K Exhibit 99.1",
    summary:
      "Foundation-model vendor. Superlative language around model quality and 'record' enterprise adoption.",
  },
  {
    id: "ec_cldl_fy25",
    ticker: "CLDL",
    ...
    sector: "software_ai",
    period: "Q4 FY2025",
    callDate: "2026-02-19T22:00:00Z",
    cik: "0001640129",
    docs: ["transcript", "press_release", "10-K", "audio"],
    executiveCount: 4,
    analystCount: 8,
    wordCount: 10420,
    source: "Motley Fool archive",
    summary:
      "Cloud reliability vendor. Uptime and margin claims; risk factor on regional capacity newly added.",
    audioVintage: "2022 (MAEC-aligned)",
  },
  {
    id: "ec_hlxt_q2fy26",
    ticker: "HLXT",
    ...
    source: "Motley Fool archive",
    summary:
      "Clinical-stage biotech. Phase 2 readout framed strongly; Q&A pressure on endpoint definitions.",
  },
  {
    id: "ec_mrdn_q3fy25",
    ticker: "MRDN",
    ...
    source: "8-K Exhibit 99.1",
    summary:
      "Specialty pharma. Launch-trajectory claims; drift from prior-year commitments on a pipeline asset.",
    audioVintage: "2021 (MDRM-aligned)",
  },
  {
    id: "ec_bcn_q4fy25",
    ticker: "BCN",
    ...
    sector: "consumer",
    period: "Q4 FY2025",
    callDate: "2026-03-12T13:30:00Z",
    cik: "0000034088",
    docs: ["transcript", "press_release", "10-K"],
    executiveCount: 3,
    analystCount: 10,
    wordCount: 12010,
    source: "Motley Fool archive",
    summary:
      "Omnichannel retailer. Same-store growth and sustainability messaging; non-GAAP margin bridge.",
  },
  {
    id: "ec_nwnd_q2fy26",
    ticker: "NWND",
    ...
    source: "Motley Fool archive",
    summary:
      "Diversified energy. Record-margin language; XBRL-verifiable revenue and operating figures.",
    audioVintage: "current webcast replay",
  },
  {
    id: "ec_vrtd_q1fy26",
    ...
    source: "8-K Exhibit 99.1",
    summary:
      "Automation hardware. Capability claims on a new robotics line; backlog framed as demand proof.",
  },
  {
    id: "ec_grnf_fy25",
    ticker: "GRNF",
    company: "Greenfield Grid",
    sector: "industrial",
    period: "Q4 FY2025",
    callDate: "2026-02-27T22:30:00Z",
    cik: "0001326801",
    docs: ["transcript", "press_release", "10-K", "audio"],
    executiveCount: 3,
    analystCount: 9,
    ...
    source: "Motley Fool archive",
    summary:
      "Grid-scale storage. '100% renewable' operations messaging; offset-accounting nuance in the 10-K.",
    audioVintage: "2022 (MAEC-aligned)",
  },
  {
    id: "ec_frwz_q3fy25",
    ticker: "FRWZ",
    company: "Freightwise Logistics",
    sector: "industrial",
    period: "Q3 FY2025",
    callDate: "2025-10-23T20:00:00Z",
    cik: "0001045810",
    docs: ["transcript", "press_release", "10-Q"],
    executiveCount: 2,
    analystCount: 7,
    ...
    source: "Motley Fool archive",
    summary:
      "Freight-tech. Universal cost-savings claims; case studies referenced but not in filings.",
  },
]
...
export function generateLedger(record: TranscriptRecord): Ledger {
  const rng = seeded(hash(record.id))
  const pool = claimPools[record.sector]
  // Stable shuffle, take 4–5 claims.
  const order = pool
    .map((seed, i) => ({ seed, i, r: rng() }))
    .sort((a, b) => a.r - b.r)
  const take = 4 + (hash(record.id) % 2)
  const chosen = order.slice(0, Math.min(take, pool.length))

  const claims: LedgerClaim[] = chosen.map(({ seed }, idx) => {
    const jitterInf = (rng() - 0.5) * 16
    const jitterGap = (rng() - 0.5) * 16
    const rhetoricalInflation = clamp(round(seed.baseInflation + jitterInf))
    const evidenceGap = clamp(round(seed.baseGap + jitterGap))
    const specificity = specificityFor(seed, rng)
    const hedgingDelta = round((seed.baseGap / 100) * 3 - specificity * 0.25 + (rng() - 0.5), 2)

    const evidence = buildEvidence(seed, evidenceGap, rng, record.cik)
    const contradicts = evidence.filter((e) => e.stance === "contradicts").length
    const found = evidence.filter((e) => e.stance !== "not_found").length

    const logit = -2.4 + 0.045 * evidenceGap + 0.02 * rhetoricalInflation
    let probability = round(1 / (1 + Math.exp(-logit)), 2)

    // Abstain when evidence is thin (few resolved items, low specificity, no contradiction).
    const thin = found <= 1 && contradicts === 0 && specificity <= 2
    let verdict: LedgerVerdict
    if (thin) {
      verdict = "abstain"
      probability = round(Math.min(probability, 0.5), 2)
    } else if (evidenceGap >= 60 || contradicts >= 2) {
      verdict = "overstated"
    } else if (evidenceGap <= 32 && rhetoricalInflation <= 45) {
      verdict = "supported"
    } else {
      verdict = "mixed"
    }

    const execIdx = Math.floor(rng() * Math.min(record.executiveCount, EXECS.length))

    return {
      id: `${record.id}_cl${idx + 1}`,
      text: seed.text,
      speaker: EXECS[execIdx],
  ...
  const evidenceItems = claims.reduce((n, c) => n + c.evidence.length, 0)
  const abstainRate = claims.length
    ? round((claims.filter((c) => c.verdict === "abstain").length / claims.length) * 100)
    : 0

  return {
    transcriptId: record.id,
    claims,
    extractedSentences: Math.round(record.wordCount / 18),
